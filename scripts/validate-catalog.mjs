...
for (const record of registryRecords) {
  if (!/modelIds:\s*\[[^\]]*\]/.test(record)) errors.push('Every OEM record must declare modelIds; an empty array means exact fitment is not proven.');
...
const refs = [...registryBody.matchAll(/manufacturerId:\s*'([^']+)'[\s\S]*?partTemplateSlug:\s*'([^']+)'[\s\S]*?referenceNumber:\s*'([^']+)'[\s\S]*?modelIds:\s*\[([^\]]*)\]/g)].map(m => ({ manufacturer: m[1], template: m[2], ref: m[3].toLowerCase().replace(/[^a-z0-9]/g, ''), models: m[4].replace(/['\s]/g, '').split(',').filter(Boolean).sort().join(',') }));
const seen = new Set();
