  ...
  return (
    <main className="min-h-screen bg-slate-100 text-slate-900">
        ...
        <div className="mx-auto max-w-7xl px-4 pb-16 pt-14 sm:px-6 lg:px-8 lg:pb-20 lg:pt-18">
          <div className="grid items-center gap-12 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="animate-fade-up">
              ...
              <div className="mt-8 flex flex-wrap gap-3"><Link href="/search" className="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-5 py-3 text-sm font-bold text-slate-950 shadow-lg shadow-sky-500/25 transition hover:-translate-y-0.5 hover:bg-sky-400">Advanced search <ArrowRight size={16} /></Link><Link href="/trucks" className="inline-flex items-center gap-2 rounded-lg border border-slate-600 bg-slate-900/60 px-5 py-3 text-sm font-bold text-slate-200 transition hover:border-sky-500/40 hover:text-sky-300"><Truck size={16} /> Select truck</Link></div>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">{TRUCK_MANUFACTURERS.slice(0, 8).map((m: (typeof TRUCK_MANUFACTURERS)[number]) => (<ManufacturerCard key={m.id} manufacturer={m} />))}</div>
