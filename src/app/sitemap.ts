...
type CatalogPart = (typeof CATALOG_PARTS)[number];
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [
    ...
    ...CATALOG_MANUFACTURERS.map((m: CatalogManufacturer) => ({ url: `${base}/trucks/${m.id}`, changeFrequency: 'weekly' as const, priority: 0.7 })),
    ...CATALOG_PARTS.slice(0, 5000).map((p: CatalogPart) => ({ url: `${base}/parts/${encodeURIComponent(p.id)}`, changeFrequency: 'monthly' as const, priority: 0.5 })),
  ];
}
